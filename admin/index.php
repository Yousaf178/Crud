...
$_SESSION['password'] = $_POST['password'];
  ...
  // echo "SELECT * FROM `admin` WHERE Email = '$email' && password = '$password' && status ='1'";exit();

// $select_query_check_mail used for mail check is this mail is already exist or not // by yousaf
    $select_query_check_mail = mysqli_query($con ,"SELECT * FROM `admin` WHERE Email = '$email' && password = '$password' && status ='1'	");
...
if(is_array($row)) {
    ...
    <!-- <script> document.location ='admin_index.php'</script>"; -->
     <?php   } else {
