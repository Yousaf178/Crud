...
if(isset($_POST['submit']))
  {
    ...
    // echo "insert into buy_product(productid,bank_number,Tax,total_price,sub_total,userid,location) value('$id','$bank_number','$tprice','$tax','$subtotal','$userid','$location')";exit();
  // Query for data insertion by yousaf
    if ($tprice > 1000) {
    	$query=mysqli_query($con, "insert into buy_product(productid,bank_number,Tax,total_price,sub_total,userid,location) value('$id','$bank_number','$tax','$tprice','$subtotal','$userid','$location')");
    }else{
    	$ttax= 0;
    	$subtotal = $tprice;
    	// echo "insert into buy_product(productid,bank_number,Tax,total_price,sub_total,userid,location) value('$id','$bank_number','$ttax','$tprice','$subtotal','$userid','$location')";
    	// echo $ttprice;exit();
     $query=mysqli_query($con, "insert into buy_product(productid,bank_number,Tax,total_price,sub_total,userid,location) value('$id','$bank_number','$ttax','$tprice','$subtotal','$userid','$location')");
    }

    if ($query) {

    	
    echo "<script>alert('ThankYou! You have successfully buy the product please wait 5 days');</script>";
    // echo "<script type='text/javascript'> document.location ='index.php'; </script>";
    
    echo "<script> document.location ='product_index.php?userid=$userid'</script>";
 }
  else
    {
